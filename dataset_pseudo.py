...
class PoseDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # scipy.misc.imsave('{0}_ori.png'.format(index), img_masked)

        # print(img_masked.shape)
        img_masked = img_masked.transpose(1, 2, 0)
        # cv2.imwrite('{0}_ori.png'.format(index), img_masked)
        img_masked = cv2.resize(img_masked, (80, 80))
        # cv2.imwrite('{0}_resize.png'.format(index), img_masked)
        img_masked = img_masked.transpose(2, 0, 1)

        # p_img = np.transpose(img_masked, (1, 2, 0))
        # scipy.misc.imsave('temp/{0}_input.png'.format(index), img_masked)
        ...
        if self.is_spherical:
            # cloud =np.hstack([cloud,pt3])
            # print("点云形状：{0}".format(cloud.shape))
        ...
        # distances = np.linalg.norm(model_points - center,axis=1)
        # sample_indices = np.argsort(distances)[:self.sample_modelpoints]
        # # print(sample_indices.shape)
        # # print(sample_indices.shape)
        # model_points = model_points[sample_indices,:]
